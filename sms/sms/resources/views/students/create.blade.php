        ...
        <div class="bg-white rounded shadow p-6">
            <form action="{{ route('students.store') }}" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="number" name="marks" value="{{ old('marks') }}" 
                        class="w-full border rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400">
                    @error('marks') <p class="text-red-500 text-sm mt-1">{{ $message }}</p> @enderror
                </div>
                <br>
                <div class="flex justify-between items-center">
                    <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2 rounded shadow">Save Student</button>
